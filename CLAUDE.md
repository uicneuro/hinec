...
2. Iterate tractography (no re-preprocess, auto-sourced from the data-layer nim):
   ...
   - DSE sweep without new YAMLs: `for h in 0.5 0.25 0.125; do ./bin/run_tractography.sh hinec_csd --score --set integrator.step=$h; done`
...
- `src/nim_tractography/` — three trackers, one shared nim:
  ...
  - `nim_tractography_template` — the ≈100-line worked example of the tracker interface (`algorithm: template`); copy it to start a new algorithm. The hand-off contract (every `nim` field and `options` field a tracker receives, layouts, and the `tracks`/`meta` return shape) is in `docs/TRACKER_INTERFACE.md`, is logged by every run via `nim_describe_tracker_input` → `<run_dir>/tractography/tracker_input.txt`, and is enforced on return by `nim_check_tracker_output`. Keep those three in step when the boundary changes.
  - Dispatch by `algorithm:` (`standard`|`hinec`|`mmf`|`template`). `field: csd` requires FOD peaks — `runTractography` computes them via `nim_csd`, cached as `<source>_csd.mat` (`csd_lmax`/`csd_max_peaks`/`csd_peak_thresh`/`csd_peak_min_sep`).
...
`config/*.yml` drives everything. Two top-level keys: `preprocessing:` and `tractography:`. Configs follow one naming rule per family (see `config/README.md`): **tracker** configs are `<algorithm>_<field>[_<variant>].yml` — `standard_dti`, `hinec_dti`, `hinec_csd`, `mmf_dti`, `mmf_csd`, plus reusable hinec DTI variants `hinec_dti_cubic` (cubic+RKF45), `hinec_dti_cubic_recall` (cubic, recall-tuned), `hinec_dti_euler` (order 1), `hinec_dti_fast` (RK2) — all for `run_tractography.sh`; **dataset** configs are `<dataset>[_variant].yml` — `ismrm2015`, `irontract` (for `run_hinec.sh`); plus `hinec_default` (the generic full-pipeline fallback). The three orthogonal axes are `algorithm` (which tracker) / `field` (`dti`|`csd`, direction source) / `integrator.method` (`euler`|`rk2`|`rk4`|`rkf45`, spelled the same way for every tracker; the old `integration_order: 1|2|4|5` is a legacy alias that still migrates). For hinec, direction is further shaped by `interpolation.method` — `trilinear` (C0), `cubic` (C1, Keys cubic convolution) or `spline` (C2). That kernel's smoothness caps how much of the integrator's formal order is reachable: RK4 measures observed order 2.00 / 3.06 / 4.00 respectively (see `docs/CONVERGENCE.md`). `sel_power` has been REMOVED — hinec is interpolation + integration only. **This is a research tool: keep useful experimental configs.** For a reusable knob-set make a `_<variant>` config; for a one-off sweep use `run_tractography.sh --set key=value` (don't delete existing configs just because a knob is expressible via `--set`). Settings irrelevant to the chosen combination are ignored by design. `src/nim_utils/nim_config_schema.m` is the single source of truth for every key (path, default, type, range, legacy aliases, help); `load_config_yaml` validates against it and `docs/YAML_CONFIG.md` is GENERATED from it by `nim_config_docs` — never hand-edit that file. Add or change knobs in the schema, not in MATLAB defaults.
...
`ARCHITECTURE.md`, `PIPELINE.md`, `PREPROCESSING.md`, `TRACTOGRAPHY.md`, `TRACKER_INTERFACE.md`, `High_Order.md`, `RKF.md`, `SEEDING_STRATEGY.md`, `YAML_CONFIG.md`, `RUN_DIRECTORY_SYSTEM.md`, `DISTRIBUTED_WORKFLOW.md`, `VISUALIZATION_GUIDE.md`, `IRONTRACT_WORKFLOW.md`, `USER_GUIDE.md`.
